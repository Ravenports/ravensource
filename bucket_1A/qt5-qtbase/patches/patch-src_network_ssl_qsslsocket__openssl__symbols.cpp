...
@@ -524,6 +524,7 @@ DEFINEFUNC(void, EC_KEY_free, EC_KEY *ec
...
@@ -769,8 +770,8 @@ static QPair<QLibrary*, QLibrary*> loadO
 #endif
...
+    libcrypto->setFileNameAndVersion(QLatin1String("%%OPENSSLLIB%%/libcrypto"), QLatin1String(SHLIB_VERSION_NUMBER));
     if (libcrypto->load() && libssl->load()) {
         ...
         return pair;
@@ -787,8 +788,8 @@ static QPair<QLibrary*, QLibrary*> loadO
...
+    libcrypto->setFileNameAndVersion(QLatin1String("%%OPENSSLLIB%%/libcrypto"), -1);
     if (libcrypto->load() && libssl->load()) {
         ...
         return pair;
@@ -994,8 +995,10 @@ bool q_resolveOpenSslSymbols()
