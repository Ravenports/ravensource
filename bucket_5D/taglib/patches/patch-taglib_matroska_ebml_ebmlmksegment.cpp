--- taglib/matroska/ebml/ebmlmksegment.cpp.orig	2026-07-05 08:55:01 UTC
...
+                                           TagLib::offset_t maxOffset)
 {
   if(offset < 0 || offset >= maxOffset) {
     return nullptr;
@@ -59,19 +59,19 @@ std::unique_ptr<ElementType> readElement
...
@@ -81,11 +81,11 @@ bool EBML::MkSegment::read(File &file)
...
@@ -93,7 +93,7 @@ bool EBML::MkSegment::readLimited(File &
...
@@ -104,10 +104,10 @@ bool EBML::MkSegment::readLimited(File &
...
+        TagLib::offset_t accPadding = 0;
         while(const auto next = findNextElement(file, maxOffset)) {
           if(next->getId() != Id::VoidElement)
             break;
@@ -124,15 +124,15 @@ bool EBML::MkSegment::readLimited(File &
...
+        matroskaSeekHead ? matroskaSeekHead->entryList() : List<std::pair<unsigned int, TagLib::offset_t>>();
